...
public class SysDictDataService : IDynamicApiController, ITransient
{
    ...
    /// 适用于列表中根据字典数据值找文本的子查询 _sysDictDataService.MapDictValueToLabel(() =>obj, obj.Type, "org_type");
    /// </summary>
    ...
    /// <returns></returns>
    public string MapDictValueToLabel<T>(T parameter, Expression<Func<object>> mappingFiled, string dictTypeCode)
    {
    ...
    /// 适用于列表数据导入根据字典数据文本找值的子查询 _sysDictDataService.MapDictLabelToValue(() =>obj, obj.Type, "org_type");
    /// </summary>
    /// <typeparam name="T"></typeparam>
    /// <param name="parameter"></param>
    /// <param name="mappingFiled"></param>
    /// <param name="dictTypeCode"></param>
    /// <returns></returns>
    public string MapDictLabelToValue<T>(T parameter, Expression<Func<object>> mappingFiled, string dictTypeCode)
    {
        return VSysDictData.InnerJoin<SysDictType>((d, dt) => d.DictTypeId.Equals(dt.Id) && dt.Code == dictTypeCode)
