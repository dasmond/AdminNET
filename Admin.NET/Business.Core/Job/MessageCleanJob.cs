...
public class MessageCleanJob: IJob
{
    ...
    public MessageCleanJob(IServiceScopeFactory scopeFactory,  ILoggerFactory loggerFactory)
    {
    ...
    public async Task ExecuteAsync(JobExecutingContext context, CancellationToken stoppingToken)
    {
        ...
        var db = serviceScope.ServiceProvider.GetRequiredService<SqlSugarRepository<TMessage>>().CopyNew();
        var sysConfigService = serviceScope.ServiceProvider.GetRequiredService<SysConfigService>();
        const int daysAgo = 10;
        await db.AsDeleteable().Where(u => u.CreateTime < DateTime.Now.AddDays(-daysAgo)).ExecuteCommandAsync(stoppingToken); // 删除消息记录
        string msg = $"【{DateTime.Now}】清理系消息成功，删除 {daysAgo} 天前的消息记录！";
