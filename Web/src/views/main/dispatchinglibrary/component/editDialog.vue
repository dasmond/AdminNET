		...
		<el-dialog v-model="isShowDialog" :width="800" draggable="">
			...
			<el-form :model="ruleForm" ref="ruleFormRef" label-width="auto" :rules="rules">
				<el-row :gutter="35">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="加工厂名称" prop="processingFactoryName">
							<el-input v-model="ruleForm.processingFactoryName" placeholder="请输入加工厂名称" maxlength="255" show-word-limit clearable />
							
						</el-form-item>
						
					</el-col>
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="加工厂id" prop="processingFactoryId">
							<el-input v-model="ruleForm.processingFactoryId" placeholder="请输入加工厂id" maxlength="19" show-word-limit clearable />
...
<script lang="ts" setup>
	...
	import { addDispatchinglibrary, updateDispatchinglibrary, detailDispatchinglibrary } from "/@/api/main/dispatchinglibrary";
	...
	const rules = ref<FormRules>({
		...
		processingFactoryName: [{required: true, message: '请输入加工厂名称！', trigger: 'blur',},],
		processingFactoryId: [{required: true, message: '请输入加工厂id！', trigger: 'blur',},],
	...
	const openDialog = async (row: any) => {
		...
		if (rowData.id)
			ruleForm.value = (await detailDispatchinglibrary(rowData.id)).data.result;
		else
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addDispatchinglibrary(values);
				} else {
					await updateDispatchinglibrary(values);
				}
				closeDialog();
			} else {
				ElMessage({
