...
<script lang="ts" setup>
	...
	import { addProcessingRejectionOrderDetails, updateProcessingRejectionOrderDetails, detailProcessingRejectionOrderDetails } from "/@/api/main/processingRejectionOrderDetails";
	...
	const openDialog = async (row: any) => {
		...
		if (rowData.id)
			ruleForm.value = (await detailProcessingRejectionOrderDetails(rowData.id)).data.result;
		else
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addProcessingRejectionOrderDetails(values);
				} else {
					await updateProcessingRejectionOrderDetails(values);
				}
				closeDialog();
			} else {
				ElMessage({
