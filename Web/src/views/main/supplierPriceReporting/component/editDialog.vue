		...
		<el-dialog v-model="isShowDialog" :width="800" draggable="">
			...
			<el-form :model="ruleForm" ref="ruleFormRef" label-width="auto" :rules="rules">
				<el-row :gutter="35">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="生效日期" prop="expireCycle">
							<el-input-number v-model="ruleForm.expireCycle" placeholder="请输入生效日期" clearable />
...
<script lang="ts" setup>
	...
	import { addSupplierPriceReporting, updateSupplierPriceReporting, detailSupplierPriceReporting } from "/@/api/main/supplierPriceReporting";
	...
	const rules = ref<FormRules>({
		...
		expireCycle: [{required: true, message: '请输入生效日期！', trigger: 'blur',},],
	...
	const openDialog = async (row: any) => {
		...
		if (rowData.id)
			ruleForm.value = (await detailSupplierPriceReporting(rowData.id)).data.result;
		else
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addSupplierPriceReporting(values);
				} else {
					await updateSupplierPriceReporting(values);
				}
				closeDialog();
			} else {
				ElMessage({
