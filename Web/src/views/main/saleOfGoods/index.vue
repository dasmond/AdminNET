  ...
  <div class="saleOfGoods-container">
    ...
    <el-card class="full-table" shadow="hover" style="margin-top: 8px">
      <el-table
        ...
        <el-table-column label="操作" width="140" align="center" fixed="right" show-overflow-tooltip="" v-if="auth('saleOfGoods:edit') || auth('saleOfGoods:delete')">
          <template #default="scope">
            <el-button icon="ele-Edit" size="small" text="" type="primary" @click="openEditSaleOfGoods(scope.row)" v-auth="'saleOfGoods:edit'"> 编辑 </el-button>
            <el-button icon="ele-Delete" size="small" text="" type="primary" @click="delSaleOfGoods(scope.row)" v-auth="'saleOfGoods:delete'"> 删除 </el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
	...
	/>
      <printDialog
        ref="printDialogRef"
        :title="printSaleOfGoodsTitle"
        @reloadTable="handleQuery" />
      <editDialog
        ref="editDialogRef"
        :title="editSaleOfGoodsTitle"
        @reloadTable="handleQuery"
      />
    </el-card>
  </div>
...
<script lang="ts" setup="" name="saleOfGoods">
  import { ref } from "vue";
  ...
  const printSaleOfGoodsTitle = ref("");
  ...
  const openAddSaleOfGoods = () => {
    editSaleOfGoodsTitle.value = '添加销售商品';
    editDialogRef.value.openDialog({});
  };

  // 打开打印页面
  const openPrintSaleOfGoods = async (row: any) => {
    printSaleOfGoodsTitle.value = '打印销售商品';
  }
  
  // 打开编辑页面
  const openEditSaleOfGoods = (row: any) => {
    editSaleOfGoodsTitle.value = '编辑销售商品';
