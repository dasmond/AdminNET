		...
		<el-dialog v-model="isShowDialog" :width="800" draggable="">
			...
			<el-form :model="ruleForm" ref="ruleFormRef" label-width="auto" :rules="rules">
				<el-row :gutter="35">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="加工厂合同编码" prop="purchaserSno">
							<el-input v-model="ruleForm.purchaserSno" placeholder="请输入加工厂合同编码" maxlength="255" show-word-limit clearable />
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="加工目标" prop="processingTarget">
							<el-input v-model="ruleForm.processingTarget" placeholder="请输入加工目标" maxlength="19" show-word-limit clearable />
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="加工总数" prop="processingQuantity">
							<el-input-number v-model="ruleForm.processingQuantity" placeholder="请输入加工总数" clearable />
...
<script lang="ts" setup>
	...
	import { addProcessingFactorySalesContractMaster, updateProcessingFactorySalesContractMaster, detailProcessingFactorySalesContractMaster } from "/@/api/main/processingFactorySalesContractMaster";
	...
	const rules = ref<FormRules>({
		...
		purchaserSno: [{required: true, message: '请输入加工厂合同编码！', trigger: 'blur',},],
		...
		processingTarget: [{required: true, message: '请输入加工目标！', trigger: 'blur',},],
		processingQuantity: [{required: true, message: '请输入加工总数！', trigger: 'blur',},],
	...
	const openDialog = async (row: any) => {
		...
		if (rowData.id)
			ruleForm.value = (await detailProcessingFactorySalesContractMaster(rowData.id)).data.result;
		else
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addProcessingFactorySalesContractMaster(values);
				} else {
					await updateProcessingFactorySalesContractMaster(values);
				}
				closeDialog();
			} else {
				ElMessage({
