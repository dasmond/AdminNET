<template>
	<div class="processingFactory-container">
		<el-dialog v-model="isShowDialog" :width="800" draggable="">
			...
			<el-form :model="ruleForm" ref="ruleFormRef" label-width="auto" :rules="rules">
				<el-row :gutter="35">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="加工厂名称" prop="name">
							<el-input v-model="ruleForm.name" placeholder="请输入加工厂名称" maxlength="255" show-word-limit clearable />
...
<script lang="ts" setup>
	...
	import { addProcessingFactory, updateProcessingFactory, detailProcessingFactory } from "/@/api/main/processingFactory";
	...
	const openDialog = async (row: any) => {
		...
		if (rowData.id)
			ruleForm.value = (await detailProcessingFactory(rowData.id)).data.result;
		else
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addProcessingFactory(values);
				} else {
					await updateProcessingFactory(values);
				}
				closeDialog();
			} else {
				ElMessage({
