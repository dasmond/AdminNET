				...
				<el-row :gutter="35">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="物料编码" prop="partNo">
							<el-input v-model="ruleForm.partNo" placeholder="请输入物料编码" maxlength="255" show-word-limit clearable />
							
						</el-form-item>
						
					</el-col>
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="生命周期阶段" prop="cycle">
							<el-input-number v-model="ruleForm.cycle" placeholder="请输入生命周期阶段" clearable />
							
						</el-form-item>
						
					</el-col>
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="工程变更通知书" prop="ecn">
							<el-input v-model="ruleForm.ecn" placeholder="请输入工程变更通知书" maxlength="255" show-word-limit clearable />
...
<script lang="ts" setup>
	...
	import { addBomMaster, updateBomMaster, detailBomMaster } from "/@/api/main/bomMaster";
	...
	const rules = ref<FormRules>({
		...
		cycle: [{required: true, message: '请输入生命周期阶段！', trigger: 'blur',},],
		ecn: [{required: true, message: '请输入工程变更通知书！', trigger: 'blur',},],
	...
	const openDialog = async (row: any) => {
		...
		if (rowData.id)
			ruleForm.value = (await detailBomMaster(rowData.id)).data.result;
		else
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addBomMaster(values);
				} else {
					await updateBomMaster(values);
				}
				closeDialog();
			} else {
				ElMessage({
