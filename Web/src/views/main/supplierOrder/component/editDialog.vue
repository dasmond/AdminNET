				...
				<el-row :gutter="35">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="入库数量" prop="quantity">
							<el-input v-model="ruleForm.quantity" placeholder="请输入入库数量" maxlength="18" show-word-limit clearable />
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="收货人" prop="consigneeName">
							<el-input v-model="ruleForm.consigneeName" placeholder="请输入收货人" maxlength="255" show-word-limit clearable />
...
<script lang="ts" setup>
	...
	import { addSupplierOrder, updateSupplierOrder, detailSupplierOrder } from "/@/api/main/supplierOrder";
	...
	const openDialog = async (row: any) => {
		...
		if (rowData.id)
			ruleForm.value = (await detailSupplierOrder(rowData.id)).data.result;
		else
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					...
					await updateSupplierOrder(values);
				}
				closeDialog();
			} else {
				ElMessage({
