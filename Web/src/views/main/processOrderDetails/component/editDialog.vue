...
<script lang="ts" setup>
	...
	import { addProcessOrderDetails, updateProcessOrderDetails, detailProcessOrderDetails } from "/@/api/main/processOrderDetails";
	...
	const openDialog = async (row: any) => {
		...
		if (rowData.id)
			ruleForm.value = (await detailProcessOrderDetails(rowData.id)).data.result;
		else
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addProcessOrderDetails(values);
				} else {
					await updateProcessOrderDetails(values);
				}
				closeDialog();
			} else {
				ElMessage({
