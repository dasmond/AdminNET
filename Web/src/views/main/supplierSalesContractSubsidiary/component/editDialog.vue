		...
		<el-dialog v-model="isShowDialog" :width="800" draggable="">
			...
			<el-form :model="ruleForm" ref="ruleFormRef" label-width="auto" :rules="rules">
				<el-row :gutter="35">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="已入库" prop="lssuedQuantity">
							<el-input-number v-model="ruleForm.lssuedQuantity" placeholder="请输入已入库" clearable />
...
<script lang="ts" setup>
	...
	import { addSupplierSalesContractSubsidiary, updateSupplierSalesContractSubsidiary, detailSupplierSalesContractSubsidiary } from "/@/api/main/supplierSalesContractSubsidiary";
	...
	const rules = ref<FormRules>({
		...
		lssuedQuantity: [{required: true, message: '请输入已入库！', trigger: 'blur',},],
	...
	const openDialog = async (row: any) => {
		...
		if (rowData.id)
			ruleForm.value = (await detailSupplierSalesContractSubsidiary(rowData.id)).data.result;
		else
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addSupplierSalesContractSubsidiary(values);
				} else {
					await updateSupplierSalesContractSubsidiary(values);
				}
				closeDialog();
			} else {
				ElMessage({
