					...
					<template #default="scope">
						<div class="effect-type-container">
							<g-sys-dict
								...
								class="m-2"
							/>
							<el-button v-if="['ApiTreeSelector', 'ForeignKey'].some((x) => scope.row.effectType == x)" @click="effectTypeChange(scope.row, scope.$index)" type="warning" :icon="Edit" link />
						</div>
					</template>
				</el-table-column>
				<el-table-column prop="dictTypeCode" label="字典" width="150" show-overflow-tooltip>
					<template #default="scope">
						<el-select v-model="scope.row.dictTypeCode" :disabled="effectTypeEnable(scope.row)" class="m-2">
							<el-option v-for="item in state.selectDataMap[scope.row.effectType] ?? []" :key="item.code" :label="item.name" :value="item.code" />
						</el-select>
					</template>
				</el-table-column>
...
const handleQuery = async (row: any) => {
	state.loading = true;
	state.tableData = await getAPI(SysCodeGenConfigApi)
		.apiSysCodeGenConfigListGet(undefined, row.id)
		.then((res) => res.data.result ?? []);
	state.loading = false;
};
