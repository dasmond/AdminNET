	...
	<el-card class="box-card" shadow="hover" style="height: 100%" body-style="height:100%; overflow:auto">
			...
			<div class="card-header">
				<div class="tree-h-flex">
					<div class="tree-h-left">
						<el-input :prefix-icon="Search" v-model="filterText" placeholder="数据资源名称" />
					</div>
		...
		<div style="margin-bottom: 45px" v-loading="state.loading">
			<el-tree
				ref="treeRef"
				class="filter-tree"
				:data="state.dataResData"
				node-key="id"
				:props="{ children: 'children', label: 'name' }"
				:filter-node-method="filterNode"
				@node-click="nodeClick"
				:show-checkbox="state.isShowCheckbox"
				:default-checked-keys="state.ownDataResData"
				highlight-current
				check-strictly
			/>
		</div>
...
const state = reactive({
	loading: false,
	dataResData: [] as Array<SysDataResource>,
...
const initTreeData = async () => {
	state.loading = true;
	var res = await getAPI(SysDataResourceApi).sysDataResourceListGet(0);
	state.dataResData = res.data.result ?? [];
	state.loading = false;
};

// 设置默认选择
const setCheckedKeys = (dataResData: any) => {
	state.isShowCheckbox = true;
	treeRef.value!.setCheckedKeys([]);
	state.ownDataResData = dataResData;
};
...
//const dataResTreeData = state.dataResData; // 异步数据导出不了？
// 导出
