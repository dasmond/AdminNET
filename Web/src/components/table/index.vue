<template>
  <div class="table-container">
          ...
          <template #default>
            ...
            <el-scrollbar>
              <div ref="toolSetRef" class="tool-sortable">
                <div class="tool-sortable-item" v-for="v in columns" :key="v.prop" v-show="!v.hideCheck && !v.fixed"
                  :data-key="v.prop">
    ...
    <el-table ref="tableRef" :data="state.data" :border="setBorder" v-bind="$attrs" row-key="id" stripe default-expand-all
      style="width: 100%" v-loading="state.loading" :default-sort="defaultSort" @selection-change="onSelectionChange"
    ...
    <div v-if="state.showPagination" class="table-footer mt15">
      <el-pagination v-model:current-page="state.page.page" v-model:page-size="state.page.pageSize" :pager-count="5"
        :page-sizes="[10, 30, 50, 100]" :total="state.total" layout="total, sizes, prev, pager, next, jumper" background
...
const importData = (data: Array<EmptyObjectType>) => {
  ...
  exportExcel(exportData, `${props.config.exportFileName ? props.config.exportFileName : themeConfig.value.globalTitle}_${new Date().toLocaleString()}.xlsx`, setHeader.value.filter(item => { return item.type != 'action' }), '导出数据');
  state.importLoading = false;
};
// 刷新
