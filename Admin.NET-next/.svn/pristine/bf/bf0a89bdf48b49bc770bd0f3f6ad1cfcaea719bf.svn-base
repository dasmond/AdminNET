...
<script lang="ts" setup>
	...
	import { addUpperComputerProgram, updateUpperComputerProgram, detailUpperComputerProgram } from "/@/api/main/upperComputerProgram";
	...
	const openDialog = async (row: any) => {
		...
		if (rowData.id)
			ruleForm.value = (await detailUpperComputerProgram(rowData.id)).data.result;
		else
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addUpperComputerProgram(values);
				} else {
					await updateUpperComputerProgram(values);
				}
				closeDialog();
			} else {
				ElMessage({
