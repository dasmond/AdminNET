				...
				<el-row :gutter="35">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="订单id" prop="orderId">
							<el-input v-model="ruleForm.orderId" placeholder="请输入订单id" maxlength="19" show-word-limit clearable />
...
<script lang="ts" setup>
	...
	import { addSupplierRejectionOrderDetails, updateSupplierRejectionOrderDetails, detailSupplierRejectionOrderDetails } from "/@/api/main/supplierRejectionOrderDetails";
	...
	const openDialog = async (row: any) => {
		...
		if (rowData.id)
			ruleForm.value = (await detailSupplierRejectionOrderDetails(rowData.id)).data.result;
		else
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addSupplierRejectionOrderDetails(values);
				} else {
					await updateSupplierRejectionOrderDetails(values);
				}
				closeDialog();
			} else {
				ElMessage({
