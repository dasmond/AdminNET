  ...
  <div class="systemFeedback-container">
      ...
      <el-form :model="queryParams" ref="queryForm" labelWidth="90">
        <el-row>
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="6" :xl="6" class="mb10">
            <el-form-item>
              <el-button-group>
                <el-button type="primary"  icon="ele-Search" @click="handleQuery" v-auth="'systemFeedback:page'"> 查询 </el-button>
              ...
              <el-button-group style="margin-left:20px">
                <el-button type="primary" icon="ele-Plus" @click="openAddSystemFeedback" v-auth="'systemFeedback:add'"> 新增 </el-button>
    ...
    <el-card class="full-table" shadow="hover" style="margin-top: 8px">
      <el-table
        ...
        <el-table-column label="操作" width="140" align="center" fixed="right" show-overflow-tooltip="" v-if="auth('systemFeedback:edit') || auth('systemFeedback:delete')">
          <template #default="scope">
            <el-button icon="ele-Edit" size="small" text="" type="primary" @click="openEditSystemFeedback(scope.row)" v-auth="'systemFeedback:edit'"> 编辑 </el-button>
            <el-button icon="ele-Delete" size="small" text="" type="primary" @click="delSystemFeedback(scope.row)" v-auth="'systemFeedback:delete'"> 删除 </el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
	...
	/>
      <printDialog
        ref="printDialogRef"
        :title="printSystemFeedbackTitle"
        @reloadTable="handleQuery" />
      <editDialog
        ref="editDialogRef"
        :title="editSystemFeedbackTitle"
        @reloadTable="handleQuery"
      />
    </el-card>
  </div>
...
<script lang="ts" setup="" name="systemFeedback">
  import { ref } from "vue";
  ...
  import editDialog from '/@/views/main/systemFeedback/component/editDialog.vue'
  import { pageSystemFeedback, deleteSystemFeedback } from '/@/api/main/systemFeedback';
  ...
  const printSystemFeedbackTitle = ref("");
  const editSystemFeedbackTitle = ref("");
  ...
  const handleQuery = async () => {
    loading.value = true;
    var res = await pageSystemFeedback(Object.assign(queryParams.value, tableParams.value));
  ...
  // 打开新增页面
  const openAddSystemFeedback = () => {
    editSystemFeedbackTitle.value = '添加系统反馈';
    editDialogRef.value.openDialog({});
  };

  // 打开打印页面
  const openPrintSystemFeedback = async (row: any) => {
    printSystemFeedbackTitle.value = '打印系统反馈';
  }
  
  // 打开编辑页面
  const openEditSystemFeedback = (row: any) => {
    editSystemFeedbackTitle.value = '编辑系统反馈';
    editDialogRef.value.openDialog(row);
  };

  // 删除
  const delSystemFeedback = (row: any) => {
  ...
  .then(async () => {
    await deleteSystemFeedback(row);
    handleQuery();
