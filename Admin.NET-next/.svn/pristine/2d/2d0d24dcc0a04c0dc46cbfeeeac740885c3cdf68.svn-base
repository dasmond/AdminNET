...
<script lang="ts" setup>
	...
	import { addProcessingFactorySalesContractSubsidiary, updateProcessingFactorySalesContractSubsidiary, detailProcessingFactorySalesContractSubsidiary } from "/@/api/main/processingFactorySalesContractSubsidiary";
	...
	const openDialog = async (row: any) => {
		...
		if (rowData.id)
			ruleForm.value = (await detailProcessingFactorySalesContractSubsidiary(rowData.id)).data.result;
		else
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addProcessingFactorySalesContractSubsidiary(values);
				} else {
					await updateProcessingFactorySalesContractSubsidiary(values);
				}
				closeDialog();
			} else {
				ElMessage({
