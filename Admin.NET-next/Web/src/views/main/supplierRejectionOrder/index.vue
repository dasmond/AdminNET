  ...
  <div class="supplierRejectionOrder-container">
    <el-card shadow="hover" :body-style="{ paddingBottom: '0' }">
      <el-form :model="queryParams" ref="queryForm" labelWidth="90">
        <el-row>
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="8" :xl="4" class="mb10" v-if="showAdvanceQueryUI">
            <el-form-item label="物流收货单id">
              <el-input v-model="queryParams.logisticsReceipt" clearable="" placeholder="请输入物流收货单id"/>
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="8" :xl="4" class="mb10" v-if="showAdvanceQueryUI">
            <el-form-item label="客户名称">
              <el-input v-model="queryParams.consigneeName" clearable="" placeholder="请输入客户名称"/>
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="6" :xl="6" class="mb10">
            <el-form-item>
              <el-button-group>
                <el-button type="primary"  icon="ele-Search" @click="handleQuery" v-auth="'supplierRejectionOrder:page'"> 查询 </el-button>
              ...
              <el-button-group style="margin-left:20px">
                <el-button type="primary" icon="ele-Plus" @click="openAddSupplierRejectionOrder" v-auth="'supplierRejectionOrder:add'"> 新增 </el-button>
    ...
    <el-card class="full-table" shadow="hover" style="margin-top: 8px">
      <el-table
        ...
        <el-table-column prop="logisticsReceipt" label="物流收货单id" width="105"  show-overflow-tooltip="" />
        ...
        <el-table-column prop="consigneeName" label="客户名称" width="140"  show-overflow-tooltip="" />
        ...
        <el-table-column label="操作" width="140" align="center" fixed="right" show-overflow-tooltip="" v-if="auth('supplierRejectionOrder:edit') || auth('supplierRejectionOrder:delete')">
          <template #default="scope">
            <el-button icon="ele-Edit" size="small" text="" type="primary" @click="openEditSupplierRejectionOrder(scope.row)" v-auth="'supplierRejectionOrder:edit'"> 编辑 </el-button>
            <el-button icon="ele-Delete" size="small" text="" type="primary" @click="delSupplierRejectionOrder(scope.row)" v-auth="'supplierRejectionOrder:delete'"> 删除 </el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
	...
	/>
      <printDialog
        ref="printDialogRef"
        :title="printSupplierRejectionOrderTitle"
        @reloadTable="handleQuery" />
      <editDialog
        ref="editDialogRef"
        :title="editSupplierRejectionOrderTitle"
        @reloadTable="handleQuery"
      />
    </el-card>
  </div>
...
<script lang="ts" setup="" name="supplierRejectionOrder">
  import { ref } from "vue";
  ...
  import editDialog from '/@/views/main/supplierRejectionOrder/component/editDialog.vue'
  import { pageSupplierRejectionOrder, deleteSupplierRejectionOrder } from '/@/api/main/supplierRejectionOrder';
  ...
  const printSupplierRejectionOrderTitle = ref("");
  const editSupplierRejectionOrderTitle = ref("");
  ...
  const handleQuery = async () => {
    loading.value = true;
    var res = await pageSupplierRejectionOrder(Object.assign(queryParams.value, tableParams.value));
  ...
  // 打开新增页面
  const openAddSupplierRejectionOrder = () => {
    editSupplierRejectionOrderTitle.value = '添加供应商拒收订单';
    editDialogRef.value.openDialog({});
  };

  // 打开打印页面
  const openPrintSupplierRejectionOrder = async (row: any) => {
    printSupplierRejectionOrderTitle.value = '打印供应商拒收订单';
  }
  
  // 打开编辑页面
  const openEditSupplierRejectionOrder = (row: any) => {
    editSupplierRejectionOrderTitle.value = '编辑供应商拒收订单';
    editDialogRef.value.openDialog(row);
  };

  // 删除
  const delSupplierRejectionOrder = (row: any) => {
  ...
  .then(async () => {
    await deleteSupplierRejectionOrder(row);
    handleQuery();
