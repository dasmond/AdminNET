<template>
  <div class="supplierContacts-container">
      ...
      <el-form :model="queryParams" ref="queryForm" labelWidth="90">
        <el-row>
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="6" :xl="6" class="mb10">
            <el-form-item>
              <el-button-group>
                <el-button type="primary"  icon="ele-Search" @click="handleQuery" v-auth="'supplierContacts:page'"> 查询 </el-button>
              ...
              <el-button-group style="margin-left:20px">
                <el-button type="primary" icon="ele-Plus" @click="openAddSupplierContacts" v-auth="'supplierContacts:add'"> 新增 </el-button>
    ...
    <el-card class="full-table" shadow="hover" style="margin-top: 8px">
      <el-table
        ...
        <el-table-column label="操作" width="140" align="center" fixed="right" show-overflow-tooltip="" v-if="auth('supplierContacts:edit') || auth('supplierContacts:delete')">
          <template #default="scope">
            <el-button icon="ele-Edit" size="small" text="" type="primary" @click="openEditSupplierContacts(scope.row)" v-auth="'supplierContacts:edit'"> 编辑 </el-button>
            <el-button icon="ele-Delete" size="small" text="" type="primary" @click="delSupplierContacts(scope.row)" v-auth="'supplierContacts:delete'"> 删除 </el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
	...
	/>
      <printDialog
        ref="printDialogRef"
        :title="printSupplierContactsTitle"
        @reloadTable="handleQuery" />
      <editDialog
        ref="editDialogRef"
        :title="editSupplierContactsTitle"
        @reloadTable="handleQuery"
      />
    </el-card>
  </div>
...
<script lang="ts" setup="" name="supplierContacts">
  import { ref } from "vue";
  ...
  import editDialog from '/@/views/main/supplierContacts/component/editDialog.vue'
  import { pageSupplierContacts, deleteSupplierContacts } from '/@/api/main/supplierContacts';
  ...
  const printSupplierContactsTitle = ref("");
  const editSupplierContactsTitle = ref("");
  ...
  const handleQuery = async () => {
    loading.value = true;
    var res = await pageSupplierContacts(Object.assign(queryParams.value, tableParams.value));
  ...
  // 打开新增页面
  const openAddSupplierContacts = () => {
    editSupplierContactsTitle.value = '添加供应商联系人资料';
    editDialogRef.value.openDialog({});
  };

  // 打开打印页面
  const openPrintSupplierContacts = async (row: any) => {
    printSupplierContactsTitle.value = '打印供应商联系人资料';
  }
  
  // 打开编辑页面
  const openEditSupplierContacts = (row: any) => {
    editSupplierContactsTitle.value = '编辑供应商联系人资料';
    editDialogRef.value.openDialog(row);
  };

  // 删除
  const delSupplierContacts = (row: any) => {
  ...
  .then(async () => {
    await deleteSupplierContacts(row);
    handleQuery();
