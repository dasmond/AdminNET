<template>
	<div class="saleOrderDetails-container">
...
<script lang="ts" setup>
	...
	import { addSaleOrderDetails, updateSaleOrderDetails, detailSaleOrderDetails } from "/@/api/main/saleOrderDetails";
	...
	const openDialog = async (row: any) => {
		...
		if (rowData.id)
			ruleForm.value = (await detailSaleOrderDetails(rowData.id)).data.result;
		else
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addSaleOrderDetails(values);
				} else {
					await updateSaleOrderDetails(values);
				}
				closeDialog();
			} else {
				ElMessage({
