  ...
  <div class="salesContractSubsidiary-container">
      ...
      <el-form :model="queryParams" ref="queryForm" labelWidth="90">
        <el-row>
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="6" :xl="6" class="mb10">
            <el-form-item>
              <el-button-group>
                <el-button type="primary"  icon="ele-Search" @click="handleQuery" v-auth="'salesContractSubsidiary:page'"> 查询 </el-button>
              ...
              <el-button-group style="margin-left:20px">
                <el-button type="primary" icon="ele-Plus" @click="openAddSalesContractSubsidiary" v-auth="'salesContractSubsidiary:add'"> 新增 </el-button>
    ...
    <el-card class="full-table" shadow="hover" style="margin-top: 8px">
      <el-table
        ...
        <el-table-column label="操作" width="140" align="center" fixed="right" show-overflow-tooltip="" v-if="auth('salesContractSubsidiary:edit') || auth('salesContractSubsidiary:delete')">
          <template #default="scope">
            <el-button icon="ele-Edit" size="small" text="" type="primary" @click="openEditSalesContractSubsidiary(scope.row)" v-auth="'salesContractSubsidiary:edit'"> 编辑 </el-button>
            <el-button icon="ele-Delete" size="small" text="" type="primary" @click="delSalesContractSubsidiary(scope.row)" v-auth="'salesContractSubsidiary:delete'"> 删除 </el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
	...
	/>
      <printDialog
        ref="printDialogRef"
        :title="printSalesContractSubsidiaryTitle"
        @reloadTable="handleQuery" />
      <editDialog
        ref="editDialogRef"
        :title="editSalesContractSubsidiaryTitle"
        @reloadTable="handleQuery"
      />
    </el-card>
  </div>
...
<script lang="ts" setup="" name="salesContractSubsidiary">
  import { ref } from "vue";
  ...
  import editDialog from '/@/views/main/salesContractSubsidiary/component/editDialog.vue'
  import { pageSalesContractSubsidiary, deleteSalesContractSubsidiary } from '/@/api/main/salesContractSubsidiary';
  ...
  const printSalesContractSubsidiaryTitle = ref("");
  const editSalesContractSubsidiaryTitle = ref("");
  ...
  const handleQuery = async () => {
    loading.value = true;
    var res = await pageSalesContractSubsidiary(Object.assign(queryParams.value, tableParams.value));
  ...
  // 打开新增页面
  const openAddSalesContractSubsidiary = () => {
    editSalesContractSubsidiaryTitle.value = '添加销售合同从表';
    editDialogRef.value.openDialog({});
  };

  // 打开打印页面
  const openPrintSalesContractSubsidiary = async (row: any) => {
    printSalesContractSubsidiaryTitle.value = '打印销售合同从表';
  }
  
  // 打开编辑页面
  const openEditSalesContractSubsidiary = (row: any) => {
    editSalesContractSubsidiaryTitle.value = '编辑销售合同从表';
    editDialogRef.value.openDialog(row);
  };

  // 删除
  const delSalesContractSubsidiary = (row: any) => {
  ...
  .then(async () => {
    await deleteSalesContractSubsidiary(row);
    handleQuery();
