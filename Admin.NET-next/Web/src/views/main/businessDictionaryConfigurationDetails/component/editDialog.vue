				...
				<el-row :gutter="35">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="编码" prop="code">
							<el-input v-model="ruleForm.code" placeholder="请输入编码" maxlength="255" show-word-limit clearable />
...
<script lang="ts" setup>
	...
	import { addBusinessDictionaryConfigurationDetails, updateBusinessDictionaryConfigurationDetails, detailBusinessDictionaryConfigurationDetails } from "/@/api/main/businessDictionaryConfigurationDetails";
	...
	const openDialog = async (row: any) => {
		...
		if (rowData.id)
			ruleForm.value = (await detailBusinessDictionaryConfigurationDetails(rowData.id)).data.result;
		else
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addBusinessDictionaryConfigurationDetails(values);
				} else {
					await updateBusinessDictionaryConfigurationDetails(values);
				}
				closeDialog();
			} else {
				ElMessage({
