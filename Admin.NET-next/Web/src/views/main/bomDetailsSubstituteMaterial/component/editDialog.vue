...
<script lang="ts" setup>
	...
	import { addBomDetailsSubstituteMaterial, updateBomDetailsSubstituteMaterial, detailBomDetailsSubstituteMaterial } from "/@/api/main/bomDetailsSubstituteMaterial";
	...
	const openDialog = async (row: any) => {
		...
		if (rowData.id)
			ruleForm.value = (await detailBomDetailsSubstituteMaterial(rowData.id)).data.result;
		else
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addBomDetailsSubstituteMaterial(values);
				} else {
					await updateBomDetailsSubstituteMaterial(values);
				}
				closeDialog();
			} else {
				ElMessage({
