				...
				<el-row :gutter="35">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="组织名" prop="name">
							<el-input v-model="ruleForm.name" placeholder="请输入组织名" maxlength="255" show-word-limit clearable />
...
<script lang="ts" setup>
	...
	import { addCustomerOrganize, updateCustomerOrganize, detailCustomerOrganize } from "/@/api/main/customerOrganize";
	...
	const openDialog = async (row: any) => {
		...
		if (rowData.id)
			ruleForm.value = (await detailCustomerOrganize(rowData.id)).data.result;
		else
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addCustomerOrganize(values);
				} else {
					await updateCustomerOrganize(values);
				}
				closeDialog();
			} else {
				ElMessage({
