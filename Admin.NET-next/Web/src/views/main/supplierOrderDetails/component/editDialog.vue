				...
				<el-row :gutter="35">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
						<el-form-item label="入库数量" prop="quantity">
							<el-input-number v-model="ruleForm.quantity" placeholder="请输入入库数量" clearable />
...
<script lang="ts" setup>
	...
	import { addSupplierOrderDetails, updateSupplierOrderDetails, detailSupplierOrderDetails } from "/@/api/main/supplierOrderDetails";
	...
	const openDialog = async (row: any) => {
		...
		if (rowData.id)
			ruleForm.value = (await detailSupplierOrderDetails(rowData.id)).data.result;
		else
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addSupplierOrderDetails(values);
				} else {
					await updateSupplierOrderDetails(values);
				}
				closeDialog();
			} else {
				ElMessage({
