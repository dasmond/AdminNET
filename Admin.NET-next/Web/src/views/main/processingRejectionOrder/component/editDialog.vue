...
<script lang="ts" setup>
	...
	import { addProcessingRejectionOrder, updateProcessingRejectionOrder, detailProcessingRejectionOrder } from "/@/api/main/processingRejectionOrder";
	...
	const openDialog = async (row: any) => {
		...
		if (rowData.id)
			ruleForm.value = (await detailProcessingRejectionOrder(rowData.id)).data.result;
		else
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addProcessingRejectionOrder(values);
				} else {
					await updateProcessingRejectionOrder(values);
				}
				closeDialog();
			} else {
				ElMessage({
