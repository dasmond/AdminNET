...
public class SysMenuService : IDynamicApiController, ITransient
{
    ...
    /// <returns></returns>
    [DisplayName("获取登录菜单树")]
    public async Task<RouteMenuOutput> GetLoginMenuTree()
    {
        if (_userManager.SuperAdmin)
        {
            ...
            return new RouteMenuOutput
            {
                Home = "home",
                Routes = menuList.Adapt<List<MenuOutput>>()
            };
        }
        else
        {
            ...
            {
                Home = "home",
                Routes = menuTree.Adapt<List<MenuOutput>>()
            };
        }
    }
        ...
        return _userManager.SuperAdmin
            ...
            : await _sysMenuRep.AsQueryable()
                .OrderBy(u => u.OrderNo).ToTreeAsync(u => u.Children, u => u.Pid, 0,
                    menuIdList.Select(d => (object)d).ToArray()); // 角色菜单授权时
    }
    ...
    public async Task UpdateMenu(UpdateMenuInput input)
    {
        ...
        var isExist = input.Type != MenuTypeEnum.Btn
            ? await _sysMenuRep.IsAnyAsync(u =>
                u.Title == input.Title && u.Type == input.Type && u.Pid == input.Pid && u.Id != input.Id)
            : await _sysMenuRep.IsAnyAsync(u =>
                u.Permission == input.Permission && u.Type == input.Type && u.Pid == input.Pid && u.Id != input.Id);
        if (isExist)
            throw Oops.Oh(ErrorCodeEnum.D4000);
