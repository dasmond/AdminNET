...
service.interceptors.request.use(
	(config) => {
		...
		// 获取本地的 token
		const accessToken = localStg.get(accessTokenKey);
		if (accessToken) {
			// 将 token 添加到请求报文头中
			...
			if (new Date() >= exp) {
				// 获取刷新 token
				const refreshAccessToken = localStg.get(refreshAccessTokenKey);
...
service.interceptors.response.use(
	(res) => {
		...
		// 判断是否存在刷新 token，如果存在则存储在本地
			...
			localStg.set(accessTokenKey, accessToken);
			localStg.set(refreshAccessTokenKey, refreshAccessToken);
		}
	...
	(error) => {
		...
		if (error.message.indexOf('timeout') != -1) {
			...
			if (error.response.data) showErrorMsg(error.response.statusText);
			else showErrorMsg('接口路径找不到');
		}
