...
<script lang="ts" setup>
	...
	import { addBomDetails, updateBomDetails, detailBomDetails } from "/@/api/main/bomDetails";
	...
	const openDialog = async (row: any) => {
		...
		if (rowData.id)
			ruleForm.value = (await detailBomDetails(rowData.id)).data.result;
		else
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addBomDetails(values);
				} else {
					await updateBomDetails(values);
				}
				closeDialog();
			} else {
				ElMessage({
