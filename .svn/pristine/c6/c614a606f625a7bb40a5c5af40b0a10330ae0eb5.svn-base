  ...
  <div class="bomDetailsCopy-container">
        ...
        <el-row>
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="8" :xl="4" class="mb10" v-if="showAdvanceQueryUI">
            <el-form-item label="变更状态">
              <el-input-number v-model="queryParams.changeStatus"  clearable="" placeholder="请输入变更状态"/>
              
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="12" :md="12" :lg="8" :xl="4" class="mb10" v-if="showAdvanceQueryUI">
            <el-form-item label="替代料状态">
              <el-input-number v-model="queryParams.alternativeMaterialStatus"  clearable="" placeholder="请输入替代料状态"/>
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="6" :xl="6" class="mb10">
            <el-form-item>
              <el-button-group>
                <el-button type="primary"  icon="ele-Search" @click="handleQuery" v-auth="'bomDetailsCopy:page'"> 查询 </el-button>
              ...
              <el-button-group style="margin-left:20px">
                <el-button type="primary" icon="ele-Plus" @click="openAddBomDetailsCopy" v-auth="'bomDetailsCopy:add'"> 新增 </el-button>
    ...
    <el-card class="full-table" shadow="hover" style="margin-top: 8px">
      <el-table
        ...
        <el-table-column prop="changeStatus" label="变更状态" width="140"  show-overflow-tooltip="" />
        <el-table-column prop="alternativeMaterialStatus" label="替代料状态" width="140"  show-overflow-tooltip="" />
        <el-table-column prop="reVision" label="乐观锁" width="140"  show-overflow-tooltip="" />
        <el-table-column label="操作" width="140" align="center" fixed="right" show-overflow-tooltip="" v-if="auth('bomDetailsCopy:edit') || auth('bomDetailsCopy:delete')">
          <template #default="scope">
            <el-button icon="ele-Edit" size="small" text="" type="primary" @click="openEditBomDetailsCopy(scope.row)" v-auth="'bomDetailsCopy:edit'"> 编辑 </el-button>
            <el-button icon="ele-Delete" size="small" text="" type="primary" @click="delBomDetailsCopy(scope.row)" v-auth="'bomDetailsCopy:delete'"> 删除 </el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
	...
	/>
      <printDialog
        ref="printDialogRef"
        :title="printBomDetailsCopyTitle"
        @reloadTable="handleQuery" />
      <editDialog
        ref="editDialogRef"
        :title="editBomDetailsCopyTitle"
        @reloadTable="handleQuery"
      />
    </el-card>
  </div>
...
<script lang="ts" setup="" name="bomDetailsCopy">
  import { ref } from "vue";
  ...
  import editDialog from '/@/views/main/bomDetailsCopy/component/editDialog.vue'
  import { pageBomDetailsCopy, deleteBomDetailsCopy } from '/@/api/main/bomDetailsCopy';
  ...
  const printBomDetailsCopyTitle = ref("");
  const editBomDetailsCopyTitle = ref("");
  ...
  const handleQuery = async () => {
    loading.value = true;
    var res = await pageBomDetailsCopy(Object.assign(queryParams.value, tableParams.value));
  ...
  // 打开新增页面
  const openAddBomDetailsCopy = () => {
    editBomDetailsCopyTitle.value = '添加BOM资料表副本';
    editDialogRef.value.openDialog({});
  };

  // 打开打印页面
  const openPrintBomDetailsCopy = async (row: any) => {
    printBomDetailsCopyTitle.value = '打印BOM资料表副本';
  }
  
  // 打开编辑页面
  const openEditBomDetailsCopy = (row: any) => {
    editBomDetailsCopyTitle.value = '编辑BOM资料表副本';
    editDialogRef.value.openDialog(row);
  };

  // 删除
  const delBomDetailsCopy = (row: any) => {
  ...
  .then(async () => {
    await deleteBomDetailsCopy(row);
    handleQuery();
