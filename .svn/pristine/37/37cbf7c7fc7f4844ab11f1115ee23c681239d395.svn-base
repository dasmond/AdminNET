<template>
  <div class="processOrder-container">
        ...
        <el-row>
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="8" :xl="4" class="mb10" v-if="showAdvanceQueryUI">
            <el-form-item label="类型">
              <el-input v-model="queryParams.typesOf" clearable="" placeholder="请输入类型"/>
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="6" :xl="6" class="mb10">
            <el-form-item>
              <el-button-group>
                <el-button type="primary"  icon="ele-Search" @click="handleQuery" v-auth="'processOrder:page'"> 查询 </el-button>
              ...
              <el-button-group style="margin-left:20px">
                <el-button type="primary" icon="ele-Plus" @click="openAddProcessOrder" v-auth="'processOrder:add'"> 新增 </el-button>
    ...
    <el-card class="full-table" shadow="hover" style="margin-top: 8px">
      <el-table
        ...
        <el-table-column label="操作" width="140" align="center" fixed="right" show-overflow-tooltip="" v-if="auth('processOrder:edit') || auth('processOrder:delete')">
          <template #default="scope">
            <el-button icon="ele-Edit" size="small" text="" type="primary" @click="openEditProcessOrder(scope.row)" v-auth="'processOrder:edit'"> 编辑 </el-button>
            <el-button icon="ele-Delete" size="small" text="" type="primary" @click="delProcessOrder(scope.row)" v-auth="'processOrder:delete'"> 删除 </el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
	...
	/>
      <printDialog
        ref="printDialogRef"
        :title="printProcessOrderTitle"
        @reloadTable="handleQuery" />
      <editDialog
        ref="editDialogRef"
        :title="editProcessOrderTitle"
        @reloadTable="handleQuery"
      />
    </el-card>
  </div>
...
<script lang="ts" setup="" name="processOrder">
  import { ref } from "vue";
  ...
  import editDialog from '/@/views/main/processOrder/component/editDialog.vue'
  import { pageProcessOrder, deleteProcessOrder } from '/@/api/main/processOrder';
  ...
  const printProcessOrderTitle = ref("");
  const editProcessOrderTitle = ref("");
  ...
  const handleQuery = async () => {
    loading.value = true;
    var res = await pageProcessOrder(Object.assign(queryParams.value, tableParams.value));
  ...
  // 打开新增页面
  const openAddProcessOrder = () => {
    editProcessOrderTitle.value = '添加加工订单';
    editDialogRef.value.openDialog({});
  };

  // 打开打印页面
  const openPrintProcessOrder = async (row: any) => {
    printProcessOrderTitle.value = '打印加工订单';
  }
  
  // 打开编辑页面
  const openEditProcessOrder = (row: any) => {
    editProcessOrderTitle.value = '编辑加工订单';
    editDialogRef.value.openDialog(row);
  };

  // 删除
  const delProcessOrder = (row: any) => {
